...
public interface CoursRepository extends JpaRepository<Cours,String> {
    ...
    @Query("SELECT c FROM Cours c WHERE c.emploiDuTemps.id = :idEdt")
    List<Cours> findByEmploiDuTempsIdAndMatiereFiliereId(@Param("idEdt") String idEmploiDuTemps, String idFiliere);
}
